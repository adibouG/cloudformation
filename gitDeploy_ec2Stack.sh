...
if [ -z "${LBAPATH}" ]
then
    echo "lba path variable is missing."
        exit 1 
fi
...
STACK_NAME="ec2-${APPNAME}-${ENV}"
        ...
        if ! (sudo cp ./gitPackerEc2AMIBuilderParameters.json ./packerEc2AMIBuilderParameters_${APPNAME}-${ENV}.json) ; then
            cp ./gitPackerEc2AMIBuilderParameters.json ./packerEc2AMIBuilderParameters_${APPNAME}-${ENV}.json
        fi
        
        sed -i'.backup' -e "s/{ECTYPE}/${ECTYPE}/g" packerEc2AMIBuilderParameters_${APPNAME}-${ENV}.json
        sed -i'.backup' -e "s/{AWSACCESSKEY}/${AWS_ACCESS_KEY}/g" packerEc2AMIBuilderParameters_${APPNAME}-${ENV}.json
        sed -i'.backup' -e "s/{AWSSECRETKEY}/${AWS_SECRET_KEY}/g" packerEc2AMIBuilderParameters_${APPNAME}-${ENV}.json
        sed -i'.backup' -e "s|{GITURL}|${CI_REPOSITORY_URL}|g" packerEc2AMIBuilderParameters_${APPNAME}-${ENV}.json
